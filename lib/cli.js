...
Usage:
    [STDIN |] enwire [options] [-- <command>]

    STDIN               enwire can read input from standard input
    options             see supported options below
    command             if specified, enwire will execute command with computed env vars

Examples:
    cat .env | enwire -- node app.js
    cat package.json | enwire
    cat package.json | enwire --no-process
    cat package.json | enwire --no-process --format=env
    echo "{\"hello\": \"world\"}" | enwire -- printenv hello

Options:
    -d, --delete        environment variable to delete
    -e, --eval          evaluate CLI argumens as JS template strings
    -i, --import        import extra .json or .env file with and merge into env vars
    -r, --rewire        from-to mapping of environment variable, e.g. --rewire db:PGDATABASE
    --delete-rewired    if specified, rewired environment variables will be deleted
    --format            by default exports in JSON format, --format=env can be set to
    ...
    --pick              environment variable to delete
                        export in env var format
    -h, --help          show this output
    -v, --version       print version of enwire
`);
};
